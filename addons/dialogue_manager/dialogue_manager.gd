...
func show_custom_dialogue_balloon(balloon_scene_path, resource: DialogueResource, title: String = "", extra_game_states: Array = []) -> Node:
	#var balloon_path: String = DialogueSettings.get_setting(&"balloon_path", balloon_scene_path)
	#if not ResourceLoader.exists(balloon_path):
		#balloon_path = balloon_scene_path
	dialogue_started.emit(resource)
	return show_dialogue_balloon_scene(balloon_scene_path, resource, title, extra_game_states)
