...
@onready var scrollingTextScene = preload("res://Menu/credits-scene/scrollingText.tscn")
...
@export var skipControl = "ui_skip"
...
func _ready():
	...
	scrollingContainer.position.y = viewSize.y*2 #0.1 #2 # it's multiplied by two as a workaround. Look at _process function
	regularSpeed = speed
	call_deferred("_position_container_offscreen")
	
	# Set title image if there is one, otherwise delete the useless node
	if titleImage != null:
		#titleImage.set_anchors_preset(Control.LayoutPreset.PRESET_FULL_RECT)
		#titleImg.set_anchors_preset(Control.LayoutPreset.PRESET_FULL_RECT)
	...
	for i in lines.size():
		...
		if line.begins_with("{") and line.ends_with("}"):
			...
			centeredText.add_theme_font_size_override("font_size", desired_size)
			if customFont != null:
			...
			if scrollingText == null:
				...
				titles.add_theme_font_size_override("font_size", desired_size)
				names.add_theme_color_override("font_color",namesColor)
				names.add_theme_font_size_override("font_size", desired_size)
...
func _position_container_offscreen() -> void:
	...
	scrollingContainer.position = Vector2(0, vp_h)
...
func _process(delta):
	...
	#if is_first_frame:
		#viewSize = get_viewport().size
		#scrollingContainer.position.y = viewSize.y * 0.1
...
func _show_skip_hint() -> void:
	...
	hint_timer.start(hint_lifetime)
	
func _on_hint_timer_timeout() -> void:
	var tween = get_tree().create_tween()
	tween.tween_property(skip_hint, "modulate:a", 0.0, fade_time)
	tween.tween_callback(Callable(skip_hint, "hide"))
