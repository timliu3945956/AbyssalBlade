...
@onready var sub_menu_button: Button = $Control/SubMenuButton
...
#@onready var gem_count_1: Sprite2D = $Control/PanelContainer/MarginContainer/VBoxContainer/Save1/SwordSprit1/GemCount1
#@onready var gem_count_2: Sprite2D = $Control/PanelContainer/MarginContainer/VBoxContainer/Save2/SwordSprite2/GemCount2
#@onready var gem_count_3: Sprite2D = $Control/PanelContainer/MarginContainer/VBoxContainer/Save3/SwordSprite3/GemCount3
const GEM_TEXTURES = [
...
func _input(event: InputEvent) -> void:
	...
	elif event is InputEventJoypadButton and not event.echo and event.pressed and press_any_button.visible == true:
		_on_press_any_button_pressed()
	...
	if event_rest == false:
		...
		if event.is_pressed() and event_rest:
			...
			event_rest = true
	if event.is_action_pressed("interact") and sub_menu_button.visible:
...
func gem_texture_for(slot_index:int) -> Texture2D:
	...
	if beaten >= BOSS_FLAGS.size(): # and Global.player_data_slots[slot_index].game_clear
...
func _on_save_1_pressed() -> void:
	...
	AudioPlayer.play_FX(file_select_fx, -10)
