...
func enter():
	...
	if Global.player_data.first_play_4 == true:
		Global.player_data.first_play_4 = false
		Global.save_data(Global.SAVE_DIR + Global.SAVE_FILE_NAME)
	animation_player.play("death")
	await animation_player.animation_finished
	owner.death_particles.play("default")
	await owner.death_particles.animation_finished
	queue_free()
