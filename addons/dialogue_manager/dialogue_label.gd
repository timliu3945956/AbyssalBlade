...
@onready var npc_talk: AudioStreamPlayer2D = $"../../../../../npc_talk"
...
@export var skip_action: StringName = &"interact" #ui_cancel
...
func type_out() -> void:
	...
	self.is_typing = true
	#AudioPlayer.play_FX(npc_talk.stream, -5)
...
func _type_next(delta: float, seconds_needed: float) -> void:
	...
	if _last_wait_index != visible_characters and additional_waiting_seconds > 0:
		...
		npc_talk.pitch_scale = randf_range(0.9, 1.1)
		npc_talk.play()
