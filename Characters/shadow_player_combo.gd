...
#@onready var smoke: AnimatedSprite2D = $smoke
#@onready var spawn_shadow_audio: AudioStreamPlayer2D = $SpawnShadowAudio
#@onready var glitch_effect_duration_timer: Timer = $GlitchEffectDurationTimer
#
#var slices_to_visit = []
#var num_slices = 8
#var slice_positions = []
...
#signal action_completed(slice_indices)
#
#var total_time_per_step: float = 3
#var time_started_movement: float = 0.0
#var time_taken_to_move: float = 0.0
#var time_taken_for_attack: float = 0.0
...
#
#func setup_movement_sequence():
	#selected_indices = slices_to_visit.duplicate()
	#movement_sequence = []
...
#func move_to_next_target():
		...
		#time_started_movement = Time.get_ticks_msec() / 1000.0
		#state = MOVE
		#if current_target_index == 2 and is_wrong_shadow:
			##start_glitch_effect()
			#glitch_effect_duration_timer.start()
	#else:
		#emit_signal("action_completed", selected_indices)
		#smoke.play("smoke")
		#await get_tree().create_timer(0.0833).timeout
...
#func move_state(delta):
	...
	## Check if arrived at target position
		...
		#time_taken_to_move = Time.get_ticks_msec() / 1000.0 - time_started_movement
		#if is_wrong_shadow:
			#print("transfer to attack state")
		#state = ATTACK
		#attack()
		#
#func attack():
		...
		#state_machine.travel("BossAttack")
		#await get_tree().create_timer(0.95).timeout
		#print("boss attacking instead clone")
	#else:
		#animation_tree.set("parameters/Attack/blend_position", target_position - position)
		#state_machine.travel("Attack")
		#await get_tree().create_timer(0.4165).timeout
...
#func _on_idle_timer_timeout():
	...
	#move_to_next_target()
#
#func _on_glitch_effect_duration_timer_timeout() -> void:
	#smoke.play("smoke")
	#spawn_shadow_audio.play()
